...
void Climber::Periodic() {
    ...
    frc::SmartDashboard::PutNumber("Climber Setpoint", m_setpointTurns.value());
...
frc2::CommandPtr Climber::Climb() {
    return frc2::cmd::RunOnce([this] {
        m_setpointTurns = kClimberEndTurns;
    });
}

frc2::CommandPtr Climber::CancelClimb() {
    return frc2::cmd::RunOnce([this] {
        m_setpointTurns = kClimberStartTurns;
    });
}

frc2::CommandPtr Climber::Stow() {
    return frc2::cmd::RunOnce([this] {
        m_setpointTurns = kClimberStowTunrs;
    });
}

void Climber::SetMotorVoltage() {
    double value = m_elevatorPID.Calculate(CurrentTurns(), m_setpointTurns);
