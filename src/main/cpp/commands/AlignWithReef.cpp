...
void AlignWithReef::Initialize() {
    ...
    if (targetPose) {
        ...
        frc::SmartDashboard::PutNumber("Start Target X", units::foot_t(m_targetPose->X()).value());
        frc::SmartDashboard::PutNumber("Start Target Y", units::foot_t(m_targetPose->Y()).value());
        frc::SmartDashboard::PutNumber("Start Target Z", units::foot_t(m_targetPose->Z()).value());
        m_targetPose = m_targetPose->TransformBy(kCenterAlignedTransform);
    } else {
...
void AlignWithReef::Execute() {
    if (m_targetPose) {
        ...
        const frc::ChassisSpeeds pidSpeeds = m_holonomicPID.Calculate(currentPose, m_targetPose->ToPose2d(), kMaxLinearVelocity, desiredHeading);

        frc::SmartDashboard::PutNumber("Target X", units::foot_t(m_targetPose->X()).value());
        frc::SmartDashboard::PutNumber("Target Y", units::foot_t(m_targetPose->Y()).value());
        frc::SmartDashboard::PutNumber("Target Z", units::foot_t(m_targetPose->Z()).value());
...
bool AlignWithReef::IsFinished() {
    ...
    units::radian_t rotational_diff = units::math::abs(currentPose.Rotation().Radians() - m_targetPose->ToPose2d().Rotation().Radians());
